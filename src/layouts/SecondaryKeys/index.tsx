...
const SecondaryKeys = () => {
  ...
  const [keyToRemove, setKeyToRemove] = useState<ISecondaryKeyData | null>(
    null,
  );
  ...
  const refreshSecondaryKeys = useCallback(async () => {
    ...
    const { data: secondaryAccountsData } =
      await identity.getSecondaryAccounts();
    const keysWithPermissions = secondaryAccountsData.map(
      convertSdkPermissionsToKeyData,
    );
