...
export const SecondaryKeyItem = ({
  ...
  const formatPortfolioDisplay = useCallback(
        ...
        <PermissionDetails>
          ...
          {data.permissions.assets.type !== 'Whole' &&
            ...
            data.permissions.assets.values.length > 0 && (
              <PermissionDetailItem>
                <PermissionDetailLabel>
                  {data.permissions.assets.type === 'These'
                    ? 'Included Assets:'
                    : 'Excluded Assets:'}
                </PermissionDetailLabel>
                <PermissionDetailList>
                ...
                <PermissionDetailList>
                  {data.permissions.transactions.values.map((tx, idx) => (
                    <PermissionDetailValue key={idx}>
                      {tx.pallet}
                      {tx.extrinsics && tx.extrinsics.length > 0
                        ? `.${tx.extrinsics.join(', ')}`
                        : ''}
                    </PermissionDetailValue>
                  ))}
          ...
          {data.permissions.portfolios.type !== 'Whole' &&
            ...
            data.permissions.portfolios.values.length > 0 && (
              <PermissionDetailItem>
                <PermissionDetailLabel>
                  {data.permissions.portfolios.type === 'These'
                    ? 'Included Portfolios:'
                    : 'Excluded Portfolios:'}
                </PermissionDetailLabel>
                <PermissionDetailList>
